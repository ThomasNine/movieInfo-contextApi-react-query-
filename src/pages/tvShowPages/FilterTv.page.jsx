...
import useFilteredTv from "../../hook/tv/useFilteredTv";
...
const FilterTvPage = () => {
  ...
  const { data, isLoading, isError, error, isSuccess } = useFilteredTv(
    currentPage,
    genreIdStr,
    releaseYr
  );
          ...
          <div className="flex space-x-3 items-center">
            ...
            <div className=" space-x-1">
              {selectedGenre?.map((each) => (
                <span
                  key={each.name}
                  className="bg-orange-600 text-white text-sm py-[1px] px-1 rounded-full"
                >
                  {each.name}
                </span>
              ))}
